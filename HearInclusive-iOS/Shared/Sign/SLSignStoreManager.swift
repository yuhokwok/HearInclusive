...
struct SLSignStoreManager {
    ...
    func fetchSigns(words : [String], completion: @escaping ([CKRecord.ID : SLSign]?, FetchError) -> Void) {
        let container = CKContainer(identifier: "iCloud.com.same.ios.HearInclusive")
        ...
        for word in words {
            let pred = NSPredicate(format: "name == %@", word)
...
//        let pred = NSPredicate(format: "name == %@", words[0], "天氣")
//        let sort = NSSortDescriptor(key: "modificationDate", ascending: false)
//        let query = CKQuery(recordType: "SLSign", predicate: pred)
